...
import '../styles/ScrollingReviews.css';
...
const ScrollingReviews: React.FC = () => {
  ...
  const handleSwipeRight = () => {
    ...
    setTimeout(() => {
      setActiveTestimonialId(prevId => {
        const currentIndex = testimonials.findIndex(testimonial => testimonial.id === prevId);
        return testimonials[(currentIndex - 1 + testimonials.length) % testimonials.length].id;
      });
      setFade(false);
    }, 500);
  };
  ...
  return (
    <>
      <div className="testimonials-carousel">
        <h3>
          What do people <span className="highlight">think about Nursify</span>?
        </h3>
        <hr className="testimonial-divider" />
      </div>
      <div {...swipeHandlers} className={`scroller ${pauseAnimation ? 'paused' : ''}`} data-speed="slow">
